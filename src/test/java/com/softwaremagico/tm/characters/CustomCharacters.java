...
public class CustomCharacters {
	...
	public void geek() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String description = characterPlayer.getCompleteNameRepresentation() + " (" + characterPlayer.getInfo().getAge()
				+ " años)"
				+ " viste con pantalones cortos, una camiseta de algún grupo de rock olvidado y una gorra del viejo y nostálgico Space Invaders que no llega a ocultar sus ojos de origen asiático. Se dedica profesionalmente a la seguridad informática y aunque no lo dice, por la seguridad con la que habla hace evidente que es bueno en ello. Ha trabajado para multiples empresas muy conocidas del sector, pero actualmente se ha hecho autónomo y tiene una empresa propia localizada en un sótano de la ciudad que llama «El Búnker». ";
	...
	public void bill() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String description = characterPlayer.getCompleteNameRepresentation() + " (" + characterPlayer.getInfo().getAge()
				+ " años)"
				+ " es un piloto de guerra ya retirado. En sus buenos tiempos, fue capitán de un gran bombardero de cuatro motores capaz de llevar cientos de toneladas de destrucción, una de las naves insignia de algún país del norte de Europa. Aunque le gusta jactarse de su formación militar, nunca ha vivido tiempos de guerra y por tanto, su experiencia se basa en maniobras de combates y simuladores virtuales. Después de muchas horas de preparación, su carrerá se vio truncada debido a un un estúpido accidente en unas maniobras militares. En dicho accidente perdió una pierna, lo que le obligó a retirarse. A pesar de su edad, tiene una forma física envidiable, únicamente mermada por su afición a los puros. ";
	...
	public void piousGirl() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String description = characterPlayer.getCompleteNameRepresentation() + " (" + characterPlayer.getInfo().getAge()
				+ " años)"
				+ " es una persona devota y de las que coloca la religión como el centro de todo lo que hace. Un corte de pelo muy sencillo resalta sobre su pelo rubio y liso y su tez pálida. Atuendos sencillos pero resistentes caracterizan a una mujer que prefiere lo práctico a la apariencia. Gracias a sus fuertes creencias, la religión le permite prácticamente justificar todos lo que ocurre a su alrededor, por lo que no necesita nada más; lo que le otroga una vida serena y tranquila debido a ello. Ha visto -o cree haber visto- milagros delante suyo, lo que le demuestra que El Hacedor está siempre con ella y que todo tiene un plan divino. ";
	...
	public void bandGirl() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String description = characterPlayer.getCompleteNameRepresentation() + " (" + characterPlayer.getInfo().getAge()
				+ " años)"
				+ " es una amante de los coches, los motores y su velocidad. Se mueve en un mundo de carreras, apuestas y diversión. Cuando no está corriendo, pasa el tiempo ayudando a su familia en un taller mecánico, profesión que se le da bien y que le ha proporcionado una interesante cantidad de dinero. Su aspecto recuerda a la típica chica de barrio obrero de alguna pequeña ciudad del mediterráneo venida a menos. El pelo desaliñado, sus manos callosas y la falta de maquillaje indica que claramente prefiere a los coches que a los chicos. ";
	...
	public void veterinary() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String specialRules = "Tienes un buen trato con los animales y te llenan de confianza siempre que ronden a tu alrededor. iempre que tengas como amigo a algún tipo de animal y este esté a la vista, tendrás un bonificador de +2 a Voluntad";
	...
	public void teacher() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String description = characterPlayer.getCompleteNameRepresentation() + " (" + characterPlayer.getInfo().getAge()
				+ " años)"
				+ " es un hombre con buena presencia y saber vestir. Trabaja como profesor de instituto y hace gala de conocimientos en varias materias. Tiene algo que le hace especial, la gente le respeta como profesional y los que le conocen le tratan bien sin saber por qué. Es una persona con carisma y buena presencia, potenciando por su prática regular de esgrima. Gracias a su aspecto físico, su pelo castaño y sus ojos verdes típicos de la costa norte del Mediterráneo, resulta evidente que incluso en las situaciones más complicadas, cae en gracia a las mujeres. ";
	...
	public void cartomancer() throws MalformedURLException, DocumentException, IOException, InvalidXmlElementException,
		...
		String description = characterPlayer.getCompleteNameRepresentation() + " (" + characterPlayer.getInfo().getAge()
				+ " años)"
				+ " es una mujer algo entrada en años que se gana la vida prediciendo el futuro con las cartas. Ha viajado de una parte a otra del mundo con sus artes y dice que ha ayudado a grandes estrellas de Hollywood para llegar hasta lo más alto gracias a sus dotes adivinatorios. Lleva numerosas pulseras y collares con multitud de piedras preciosas y semipreciosas que junto a sus rasgos del norte de áfrica le dan un toque éxotico que le ensalza su imagen de adivina.";
		String background = "Las desgracias son siempre buenas para el negocio y a pesar del toque de queda inicialmente tu número de clientes aumentó. Esto ha ido en declive, si bien, gracias al contacto que has tenido con esta gente los últimos días, has escuchado numerosos rumores. Parece que una entidad oscura está detrás de todo esto y que los árboles negros son un puente entre el infierno y nuestro mundo. ";
		...
		String secondaryObjectives = "Siempre has sido una timadora y una ladrona, pero los eventos actuales te dan que pensar que algo sobrenatural existe en la ciudad. Has intentado buscar qué o quién está detrás de todo esto, pero a pesar de tus habilidades de sigilo, la fuerte presencia militar te ha impedido descubrir nada. Notas que hay una presencia oscura en la ciudad, y quieres contactar con ella. Te gustaría pactar o controlar lo que sea que está pasando aquí para poder ser por fin una auténtica hechicera. ";
